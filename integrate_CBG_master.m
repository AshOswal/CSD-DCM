...
for k = 1%:length(P)
    ...
    for q =  1:length(dQ)
        ...
        U.dt     = 1/U.N;
        U.time   = 1;
        U.input  = [zeros(200,2) ;[ones(1,1) 0] ;zeros(U.N-1-200,2)];
        y = integrate_CBG(qE,M,U);
        U.input  = [zeros(200,2) ;[ 0 ones(1,1)] ;zeros(U.N-1-200,2)];
        y1 = integrate_CBG(qE,M,U);
        ...
        plot(0:U.dt:(U.N-1)*U.dt,y(1,:),'r');hold on;
        plot(0:U.dt:(U.N-1)*U.dt,y(2,:),'b:');hold on;
        plot(0:U.dt:(U.N-1)*U.dt,y1(1,:),'r:');hold on;
        plot(0:U.dt:(U.N-1)*U.dt,y1(2,:),'b');hold on;
        xlabel('time(s)')
        ylabel('Amplitude');
