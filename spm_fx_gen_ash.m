function [f,J,Q] = spm_fx_gen_ash(x,u,P,M)
...
fx{3} = @spm_fx_bgt_a;                     % basal ganglia circuit
fx{4} = @spm_fx_mmc_a;                     % motor micro circuit
...
for i = 1:n
    Sx{i} = S(x{i},R,B);          % sigmoid firing rate function
end
...
try
    ...
    if strcmp(M.N,'no delay') % intergrate without delays
    Q=1;
    end
    return
end
